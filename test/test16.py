...
if __name__ == "__main__":
    # 读取数据
    data_set = pd.read_csv('./bpnn_V2/iris_training.csv', header=None)
    X = data_set.iloc[:, 0:4].values.T  # 前四列是特征，T表示转置
    Y = data_set.iloc[:, 4:7].values.T  # 后三列是标签
    Y = Y.astype('uint8')
    ...
    # 对模型进行测试
    data_test = pd.read_csv('./bpnn_V2/iris_test.csv', header=None)
    x_test = data_test.iloc[:, 0:4].values.T
    y_test = data_test.iloc[:, 4:7].values.T
